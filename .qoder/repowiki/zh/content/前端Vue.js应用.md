...
subgraph "后端"
H["GenerateController<br/>/api/generate/*"]
I["GraphWorkflowService<br/>LangGraph 执行"]
J["ProjectState<br/>工作流状态"]
K["VueGenerator<br/>Vue 代码生成节点"]
end
A --> B --> C
C --> D
C --> E
...
CHAT --> |继续发送| START
CHECK --> RESUME["调用 /api/generate/resume"]
RESUME --> GEN["生成动画"]
GEN --> RESULT["结果步骤<br/>展示代码与审查反馈"]
RESULT --> COPY["复制代码"]
RESULT --> RESET["重新开始"]
RESET --> I
```
