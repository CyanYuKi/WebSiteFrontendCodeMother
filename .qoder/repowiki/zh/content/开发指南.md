...
- 后端（Spring Boot）
  ...
  - prompt：统一的提示词模板
  - config：LangGraph状态图装配与Web配置
  - resources：应用配置与静态资源
- 前端（Vite + Vue 3 + TailwindCSS）
  - 使用Vite构建，集成TailwindCSS样式框架与Vue生态
  - 自动API代理到后端服务器，支持热重载开发体验
...
subgraph "前端(Vite+Vue+TailwindCSS)"
FE["浏览器/客户端"]
VITE["Vite开发服务器"]
...
- Web配置
  - WebConfig配置跨域支持，允许前端开发服务器访问后端API
...
## 架构总览
后端通过LangGraph4j的状态图驱动工作流，前后端通过REST API交互。前端负责用户交互与展示，后端负责AI推理与状态流转。Vite开发服务器提供自动API代理功能，将/api前缀的请求转发到后端服务器。

```mermaid
sequenceDiagram
participant FE as "前端(Vite开发服务器)"
...
FE->>PROXY : POST /api/generate/start
PROXY->>CTRL : 转发到后端
...
NODE1-->>SVC : 返回意图与回复
SVC-->>CTRL : 返回状态
CTRL-->>PROXY : {sessionId, intentType, chatReply, checklist}
PROXY-->>FE : {sessionId, intentType, chatReply, checklist}
FE->>PROXY : POST /api/generate/resume {sessionId, answers}
PROXY->>CTRL : 转发到后端
...
SVC-->>CTRL : 返回最终状态
CTRL-->>PROXY : {vueCode, reviewPassed, reviewFeedback, retryCount}
PROXY-->>FE : {vueCode, reviewPassed, reviewFeedback, retryCount}
```
...
### Web配置：WebConfig
- 职责
  - 配置跨域支持，允许前端开发服务器（localhost:5173）访问后端API
...
- 跨域问题
  - 检查WebConfig中的跨域配置，确认前端开发服务器地址是否在允许列表中
- 前端样式问题
  - 确认TailwindCSS配置正确，检查style.css中的样式定义
...
## 结论
WebsiteMother通过LangGraph4j实现了清晰的工作流编排，结合DashScope Qwen模型完成了从意图分析到Vue代码生成的自动化流程。新增的Vite开发环境配置提供了更好的开发体验，包括自动API代理、热重载和TailwindCSS样式支持。建议在开发阶段遵循本文档的开发与测试规范，生产阶段完善会话存储与日志策略，并持续优化提示词模板以提升生成质量。
...
- 前端环境
  ...
  - Vite开发服务器：自动API代理到后端服务器（localhost:8080）
  - TailwindCSS：提供现代化的样式框架支持
...
- [package.json:1-24](file://frontend/package.json#L1-L24)
- [vite.config.js:1-17](file://frontend/vite.config.js#L1-L17)
- [style.css:1-275](file://frontend/src/style.css#L1-L275)
...
- 后端
  ...
  - 访问API：启动与继续接口
- 前端
  - 安装依赖后运行开发服务器（Vite自动代理到后端）
  - 构建与预览命令参考package.json脚本
  - 使用TailwindCSS类名进行样式定制
- 开发流程
