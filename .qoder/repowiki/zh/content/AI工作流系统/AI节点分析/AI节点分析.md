...
## 更新摘要
**变更内容**
- 更新资产收集节点以反映AI驱动的资产配置系统
- 新增智能搜索查询生成和Logo生成功能的详细说明
- 添加Pexels图片搜索服务和DashScope集成
- 更新SSE进度通知机制
- 扩展资产收集流程的多服务协作模式
...
## 简介
本文件面向WebsiteMother的AI节点系统，围绕"意图识别—需求清单—AI驱动资产收集—代码生成—代码审查"的工作流，系统化梳理各AI节点的功能职责、输入输出规范、处理逻辑与协作关系。重点解析：
- IntentAnalyzer的意图识别算法与输出格式
- ChecklistBuilder的需求清单生成策略与JSON约束
- **更新** AssetCollector的AI驱动资产配置系统、智能搜索查询生成和Logo生成功能
...
- node：AI节点实现（意图分析、清单生成、AI驱动资产收集、Vue生成、代码审查）
- service：工作流执行与LLM调用封装，**新增** LogoGenerationService和PexelsImageService
- state：全局状态容器
- prompt：统一的提示词模板
- edge：条件边（路由）
- config：LangGraph4j图编排
- controller：对外HTTP接口
- resources：应用配置（包含DashScope Qwen模型密钥和Pexels API密钥）
...
LG["LogoGenerationService<br/>AI Logo生成"]
PI["PexelsImageService<br/>真实图片搜索"]
end
subgraph "状态与模板"
...
- [LogoGenerationService.java:18-20](file://src/main/java/com/example/websitemother/service/LogoGenerationService.java#L18-L20)
- [PexelsImageService.java:21-23](file://src/main/java/com/example/websitemother/service/PexelsImageService.java#L21-L23)
...
- **更新** 资产收集（AssetCollector）
  - 职责：AI驱动的智能资产配置，生成Logo和真实图片素材
  - 输入：用户答案映射（来自状态）
  - 输出：assets JSON字符串，包含Logo和多区块图片
  - 关键点：LLM生成精准搜索查询，集成DashScope和Pexels服务
...
- [AssetCollector.java:21-278](file://src/main/java/com/example/websitemother/node/AssetCollector.java#L21-L278)
...
- resumeGraph：AI驱动资产收集 → Vue生成 → 代码审查（循环），通过ReviewRouter控制重试与结束
...
WF->>G2 : invoke(state)
G2->>AssetCollector : AI驱动资产收集
AssetCollector->>ChatModelService : 生成搜索配置
AssetCollector->>LogoGenerationService : 生成Logo
AssetCollector->>PexelsImageService : 搜索真实图片
...
### **更新** 资产收集（AssetCollector）
- 功能职责
  - AI驱动的智能资产配置：生成品牌Logo和多区块图片素材
  - 基于用户需求和答案生成精准搜索查询
  - 集成DashScope通义万相生成Logo和Pexels真实图片搜索
- 输入输出
  - 输入：ProjectState.userAnswers()
  - 输出：ProjectState.ASSETS_JSON（JSON字符串）
- 处理逻辑
  - **新增** 调用LLM生成AssetConfig配置，包含品牌名、Logo提示词和搜索查询
  - **新增** 根据用户选择决定是否生成Logo
  - **新增** 按区块分别搜索图片，hero区块默认2张，其余区块1张
  - **新增** 自动回退到Pexels搜索，若无结果则使用占位图
  - **新增** SSE进度通知，实时反馈处理阶段
- 错误处理
  - LLM配置生成失败时使用兜底策略
  - API调用异常时记录日志并继续执行
...
ReadAns --> GenConfig["LLM生成AssetConfig<br/>品牌名/Logo提示词/搜索查询"]
GenConfig --> NeedLogo{"用户需要Logo?"}
NeedLogo --> |是| GenLogo["DashScope生成Logo"]
NeedLogo --> |否| SkipLogo["跳过Logo生成"]
GenLogo --> SearchImg["按区块搜索图片<br/>hero:2张, 其他:1张"]
SkipLogo --> SearchImg
SearchImg --> Pool["构建额外图片素材池"]
Pool --> HeroCheck{"hero区块有图片?"}
HeroCheck --> |否| Fallback["Pexels搜索回退<br/>占位图生成"]
HeroCheck --> |是| ToJson["序列化为JSON"]
Fallback --> ToJson
ToJson --> SSE["SSE进度通知"]
SSE --> Save["保存至ASSETS_JSON"]
Save --> End(["结束"])
```

**图表来源**
- [AssetCollector.java:68-131](file://src/main/java/com/example/websitemother/node/AssetCollector.java#L68-L131)
- [AssetCollector.java:157-211](file://src/main/java/com/example/websitemother/node/AssetCollector.java#L157-L211)
...
- **LogoGenerationService**
  - 基于阿里云DashScope的wanx-v1模型生成AI Logo
  - 支持异步任务提交和轮询查询
  - 提供自定义Prompt和预设Prompt两种生成方式
- **PexelsImageService**
  - 提供与网站主题相关的高质量真实图片搜索
  - 支持多页搜索和图片URL提取
  - 集成API密钥管理和错误处理
...
```mermaid
flowchart TD
A["AssetCollector"] --> B["LogoGenerationService<br/>DashScope异步生成"]
A --> C["PexelsImageService<br/>真实图片搜索"]
B --> D["Logo URL"]
C --> E["图片URL列表"]
D --> F["构建Assets JSON"]
E --> F
...
**图表来源**
- [LogoGenerationService.java:41-78](file://src/main/java/com/example/websitemother/service/LogoGenerationService.java#L41-L78)
- [PexelsImageService.java:40-77](file://src/main/java/com/example/websitemother/service/PexelsImageService.java#L40-L77)

**章节来源**
- [LogoGenerationService.java:14-173](file://src/main/java/com/example/websitemother/service/LogoGenerationService.java#L14-L173)
...
- 组件耦合
  ...
  - **新增** AssetCollector依赖LogoGenerationService和PexelsImageService
  - GraphConfig集中编排节点与路由，清晰分离关注点
- 状态传递
  - ProjectState作为全局载体，贯穿所有节点，字段命名统一，便于跨节点共享
- 外部依赖
  - DashScope Qwen模型通过LangChain4j封装调用
  - **新增** DashScope通义万相模型用于Logo生成
  - **新增** Pexels API用于真实图片搜索
...
AssetCollector --> ChatModelService : "生成搜索配置"
AssetCollector --> LogoGenerationService : "生成Logo"
AssetCollector --> PexelsImageService : "搜索图片"
...
## 性能考虑
- LLM调用成本控制
  - **更新** 使用SMART模型类型进行高质量资产配置生成
  - 合理设置提示词长度，避免冗余上下文
  - 在生成阶段尽量复用已有的assets JSON，减少重复描述
- **新增** AI服务性能优化
  - Logo生成采用异步任务模式，避免阻塞主线程
  - Pexels搜索限制每页数量，控制API调用频率
  - SSE进度通知减少用户等待焦虑
- 状态最小化
...
- LLM调用异常
  ...
  - 参考
    - [ChatModelService.java:146-151](file://src/main/java/com/example/websitemother/service/ChatModelService.java#L146-L151)
    - [application.yml:4-8](file://src/main/resources/application.yml#L4-L8)
- **新增** Logo生成失败
  - 现象：LogoGenerationService返回null或超时
  - 排查：确认DashScope API Key配置，检查网络连接和任务状态轮询
  - 参考
    - [LogoGenerationService.java:42-48](file://src/main/java/com/example/websitemother/service/LogoGenerationService.java#L42-L48)
    - [LogoGenerationService.java:126-171](file://src/main/java/com/example/websitemother/service/LogoGenerationService.java#L126-L171)
- **新增** 图片搜索失败
  - 现象：PexelsImageService返回空列表或异常
  - 排查：确认Pexels API Key配置，检查网络连接和搜索关键词有效性
  - 参考
    - [PexelsImageService.java:42-45](file://src/main/java/com/example/websitemother/service/PexelsImageService.java#L42-L45)
    - [PexelsImageService.java:72-74](file://src/main/java/com/example/websitemother/service/PexelsImageService.java#L72-L74)
- 意图识别结果异常
...
- **新增** 资产配置生成失败
  - 现象：AssetCollector使用兜底策略但仍缺少关键素材
  - 排查：检查LLM响应格式，确认JSON结构正确性，验证兜底查询生成
  - 参考
    - [AssetCollector.java:199-211](file://src/main/java/com/example/websitemother/node/AssetCollector.java#L199-L211)
- Vue代码生成失败
...
## 结论
WebsiteMother的AI节点系统通过LangGraph4j实现了清晰的工作流编排，**更新**后的资产收集节点集成了AI驱动的智能配置、Logo生成和真实图片搜索功能，显著提升了生成质量和用户体验。各节点职责单一、输入输出规范明确，借助统一的提示词模板与状态容器，系统具备良好的可扩展性与可维护性。建议在生产环境中强化会话持久化、监控与告警，并持续优化提示词以提升生成质量与稳定性。

## 附录
- 使用场景示例
  - 场景A：用户输入"我想做个美食分享网站"，系统识别为建站需求，返回需求清单，用户补充主题、风格、功能模块后，系统通过AI生成Logo和真实图片素材，然后生成Vue代码并通过审查
  - 场景B：用户输入"你好"，系统识别为闲聊，直接返回友好回复并结束
  - **新增** 场景C：用户需要Logo但不需要真实图片，系统仅生成Logo并回退到占位图
- 配置参数说明
  - DashScope Qwen模型配置位于application.yml，包含API密钥与模型名称
  - **新增** Pexels API密钥配置，用于真实图片搜索
  - **新增** LogoGenerationService使用DashScope的wanx-v1模型进行Logo生成
  - 参考
    - [application.yml:4-8](file://src/main/resources/application.yml#L4-L8)
    - [LogoGenerationService.java:27-28](file://src/main/java/com/example/websitemother/service/LogoGenerationService.java#L27-L28)
    - [PexelsImageService.java:27-28](file://src/main/java/com/example/websitemother/service/PexelsImageService.java#L27-L28)
- API参考
  - POST /api/generate/start：启动工作流，返回sessionId、意图类型、清单
  - POST /api/generate/resume：提交答案继续执行，返回Vue代码、审查结果与重试次数
  - **新增** SSE事件流：/api/generate/stream/{sessionId}，实时接收处理阶段通知
  - 参考
    - [GenerateController.java:33-84](file://src/main/java/com/example/websitemother/controller/GenerateController.java#L33-L84)
    - [AssetCollector.java:254-261](file://src/main/java/com/example/websitemother/node/AssetCollector.java#L254-L261)