...
- 用户提示词生成：将用户输入包裹为“用户输入：\"...\"”
- 解析策略：大小写不敏感匹配“create”，REPLY为空或“null”时转为null
- 日志记录：记录输入与识别结果，便于调试
...
CheckReply --> |是| SetReply["设置chatReply空或null"]
...
- 响应格式不符合预期
  ...
  - 排查：确认系统提示词严格要求输出INTENT与REPLY两行，且REPLY为null或空字符串时应视为null
...
- 会话状态丢失
  ...
  - 参考：[GenerateController.java:62-64](file://src/main/java/com/example/websitemother/controller/GenerateController.java#L62-L64)
- 工作流启动/恢复失败
...
- 输入
  ...
  - 示例：见[GenerateController.java:89-91](file://src/main/java/com/example/websitemother/controller/GenerateController.java#L89-L91)
- 输出
  - 响应体：StartResponse.sessionId、intentType、chatReply、checklist
  - 示例：见[GenerateController.java:94-99](file://src/main/java/com/example/websitemother/controller/GenerateController.java#L94-L99)

章节来源
- [GenerateController.java:88-115](file://src/main/java/com/example/websitemother/controller/GenerateController.java#L88-L115)
...
- REPLY解析：去除前后空白，若为“null”或空字符串则设为null
- 参考：[IntentAnalyzer.java:38-51](file://src/main/java/com/example/websitemother/node/IntentAnalyzer.java#L38-L51)
...
- 配置：调整模型参数以平衡准确率与延迟
- 参考：[application.yml:4-9](file://src/main/resources/application.yml#L4-L9)
